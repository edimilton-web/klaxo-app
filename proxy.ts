...
export default auth((req) => {
  ...
  // /welcome?session_id=... is the guest-checkout confirmation flow (no session yet); only the
  // post-signup/PWA-install landing (no session_id) requires an authenticated user.
  const isWelcomeWithoutSession = nextUrl.pathname.startsWith("/welcome") && !nextUrl.searchParams.has("session_id")
  ...
  if (!isLoggedIn && (isDashboard || isApiProtected || isWelcomeWithoutSession)) {
    if (isApiProtected) {
...
export const config = {
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|manifest.json|icons|email|blog|robots.txt|sitemap.xml).*)",
  ],
}
