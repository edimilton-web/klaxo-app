...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex h-screen w-64 flex-col border-r border-slate-200 bg-white sticky top-0">
        ...
        <nav className="flex-1 space-y-1 p-3">
          {NAV.map((item) => {
            ...
            return (
              ...
              >
                <svg className={cn("h-5 w-5 flex-shrink-0", active ? "text-violet-600" : "text-slate-400")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
        ...
        <div className="border-t border-slate-100 p-3">
          ...
          <button
            ...
            className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-500 hover:bg-slate-50 hover:text-slate-700"
          >
          ...
          <div className="mt-3 border-t border-slate-100 pt-3 px-3">
            <a href="mailto:support@klaxo.app" className="block text-xs text-slate-400 hover:text-violet-600 transition-colors">
              support@klaxo.app
            </a>
            <Link href="/privacy" className="block text-xs text-slate-400 hover:text-violet-600 transition-colors mt-0.5">
              Privacy &amp; GDPR
            </Link>
          </div>
        </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-slate-200 bg-white px-2 py-2">
        {NAV.map((item) => {
          const active = pathname === item.href || (item.href !== "/dashboard" && pathname.startsWith(item.href))
          return (
            <Link
            ...
            >
              <svg className={cn("h-5 w-5", active ? "text-violet-600" : "text-slate-400")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
                <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
              </svg>
